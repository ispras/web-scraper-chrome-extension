d3 = (function() {
	...
	function d3_event_dragSuppress() {
		var name = '.dragsuppress-' + ++d3_event_dragId,
			...
			w = d3
				.select(d3_window)
				.on(touchmove, d3_eventPreventDefault)
				.on(selectstart, d3_eventPreventDefault)
				.on(dragstart, d3_eventPreventDefault),
	...
	function d3_geo_clipAntimeridianIntersect(λ0, φ0, λ1, φ1) {
		...
		return abs(sinλ0_λ1) > ε
			? Math.atan((Math.sin(φ0) * (cosφ1 = Math.cos(φ1)) * Math.sin(λ1) - Math.sin(φ1) * (cosφ0 = Math.cos(φ0)) * Math.sin(λ0)) / (cosφ0 * cosφ1 * sinλ0_λ1))
			: (φ0 + φ1) / 2;
	}
	...
	function d3_svg_lineCardinalClosed(points, tension) {
		return points.length < 3
			...
			: points[0] + d3_svg_lineHermite((points.push(points[0]), points), d3_svg_lineCardinalTangents([points[points.length - 2]].concat(points, [points[1]]), tension));
	}
