	...
	for (var i = 0; i < resultItems.length; i++) {
		...
		if (result.type == 'log') {
			messagesDiv.appendChild(
				this.createDom('div', { className: 'resultMessage log' }, result.toString())
			);
		} else if (result.type == 'expect' && result.passed && !result.passed()) {
			messagesDiv.appendChild(
				this.createDom('div', { className: 'resultMessage fail' }, result.message)
			);
...
jasmine.TrivialReporter.prototype.reportSpecResults = function (spec) {
	...
	var specDiv = this.createDom(
		...
		this.createDom(
			'a',
			{ className: 'run_spec', href: '?spec=' + encodeURIComponent(spec.getFullName()) },
			'run'
		),
		this.createDom(
			'a',
