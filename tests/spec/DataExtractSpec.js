describe('DataExtractor', function () {
	...
	it('should be able to find selectors common to all selector trees', function () {
		...
		expect(extractor.getSelectorsCommonToAllTrees('_root')).matchSelectorList(
			expectedSelectors
		);
	});
	...
	it('should be able to find selector tree without multiple or follow elements', function () {
		...
		var expected = [['parent', 'parent2', 'child']];
		var result = extractor.findSelectorTrees();
		expect(result).matchSelectorTrees(expected);
	});
	...
	it('should return one selector tree for this sitemap', function () {
		var sitemap = Sitemap.sitemapFromObj({
			selectors: [
				{
					parentSelectors: ['_root', 'mirror-page'],
					type: 'SelectorElement',
					multiple: true,
					follow: false,
					id: 'mirror-row',
	...
	it('should test getSelectorCommonData with one selector', function () {
		...
		var deferred = extractor.getSelectorCommonData(
			sitemap.selectors,
			sitemap.selectors[0],
			parentElement
		);
	...
	it('should test getSelectorTreeCommonData with one selector', function () {
		...
		var deferred = extractor.getSelectorTreeCommonData(
			sitemap.selectors,
			'_root',
			parentElement
		);
