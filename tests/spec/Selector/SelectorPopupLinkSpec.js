describe('Popup link Selector', function () {
	...
	it('should extract multiple links', function () {
		$el.append(
			'<a onclick="window.open(\'http://example.com/a\')">a</a><a onclick="window.open(\'http://example.com/b\')">b</a>'
		);
	...
	it('should extract url from an async window.open call', function () {
		$el.append(
			$('<a onclick="setTimeout(function(){window.open(\'http://example.com/\');},100)"></a>')
		);
		var selector = new Selector({
			type: 'SelectorPopupLink',
		});
	...
	it('should getData url from an async window.open call', function () {
		$el.append(
			$(
				'<a onclick="setTimeout(function(){window.open(\'http://example.com/\');},100)">a</a>'
			)
		);
		var selector = new Selector({
			id: 'a',
